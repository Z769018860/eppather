...
int main()
{
    ...
    *proot << VarDecl::create("tmp", int_type, IntLiteral::create(1))
    ...
    root << structA;
    structA->addField(FieldDecl::create("a", int_type));
    root << ArraySubscript::create(
        DeclRefExpr::create(root.getValueDecl("a")), IntLiteral::create(1)
    );
    root << BinaryOperator::create(
        DeclRefExpr::create(root.getValueDecl("b")),
        DeclRefExpr::create(root.getValueDecl("c")), "+"
    );
    root << BreakStmt::create();
    auto f = FunctionDecl::create("f", int_type, {});
    root << f;
    *f << ParmVarDecl::create("a", int_type);
    ...
    root << MemberExpr::create(
        DeclRefExpr::create(root.getValueDecl("sa")), *structA, "a"
    );
