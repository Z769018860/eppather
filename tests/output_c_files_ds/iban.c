...
int mod97(const char *s, int len) {
    int rem[10] = {48, 48, 0, 0, 0, 0, 0, 0, 0, 0};
    int parts = len / 7;
    int i = 0;
    int j = 0;
    int num = 0;
    for (i = 1; i <= parts + (len % 7 != 0 ? 1 : 0); i = i + 1) {
        for (j = 0; j < 7 && (i - 1) * 7 + j < len; j = j + 1) {
            rem[2 + j] = s[(i - 1) * 7 + j];
        }
        num = 0;
        for (j = 0; j < 10 && rem[j] != 0; j = j + 1) {
            num = num * 10 + (rem[j] - 48);
        }
        num = num % 97;
        rem[0] = num / 10 + 48;
...
int valid_iban(char *iban) {
    ...
    int j = 0;
    for (i = 0; i < sz; i = i + 1) {
        if ((iban[i] < 48 || iban[i] > 57) && (iban[i] < 65 || iban[i] > 90)) {
            return 0;
        }
