	...
	Matrix operator * (const Matrix& a) const
	{
		...
		for (i = 0; i < vec.size(); i++)
			for (k = 0; k < vec[0].size(); k++)
			{
				...
				{
					b.vec[i][j] += temp * a.vec[k][j] % mod;
					b.vec[i][j + 1] += temp * a.vec[k][j + 1] % mod;
					b.vec[i][j + 2] += temp * a.vec[k][j + 2] % mod;
					b.vec[i][j + 3] += temp * a.vec[k][j + 3] % mod;
					b.vec[i][j + 4] += temp * a.vec[k][j + 4] % mod;
					b.vec[i][j + 5] += temp * a.vec[k][j + 5] % mod;
					b.vec[i][j + 6] += temp * a.vec[k][j + 6] % mod;
					b.vec[i][j + 7] += temp * a.vec[k][j + 7] % mod;
