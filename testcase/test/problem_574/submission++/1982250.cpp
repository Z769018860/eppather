...
struct cmp2{bool operator()(pair<int,int> a,pair<int,int> b){return b<a;}};
tree<pair<int,int>,null_type,cmp2,rb_tree_tag,tree_order_statistics_node_update> op[N];
int tr[N<<2],ls[N<<2],rs[N<<2],tag[N<<2],tr_tot,root[N];
pair<int,int> pai[2][N<<2];
pair<int,int> emp = make_pair(INT_MIN,-1);
...
inline void modify_delta(int l,int r,int c,int s,int t,int p){
	...
	tr[ls[p]]+=tag[p],tag[ls[p]]+=tag[p],tr[rs[p]]+=tag[p],tag[rs[p]]+=tag[p],tag[p]=0;
	if(l<=((s+t)>>1)) modify_delta(l,r,c,s,(s+t)>>1,ls[p]);
	if(r>((s+t)>>1)) modify_delta(l,r,c,((s+t)>>1)+1,t,rs[p]);
	tr[p]=min(tr[ls[p]],tr[rs[p]]);
}
inline void add(int x,int y,pair<int,int> c,int s,int t,int p){
	if(s==t){
		pai[y][p] = c;
		return ;
	}
	if(x<=(s+t)>>1) add(x,y,c,s,(s+t)>>1,p<<1);
	else add(x,y,c,((s+t)>>1)+1,t,(p<<1)+1);
	pai[y][p] = max(pai[y][p<<1],pai[y][(p<<1)+1]);
}
...
inline pair<int,int> query(int l,int r,int c,int s,int t,int p){
...
inline pair<int,int> found_pair_max(int k){
...
inline pair<int,int> found_list_max(int k){
...
inline void add_upd(int k){
	while(k){
		pair<int,int> tmp = found_pair_max(k);
		add(dfn[k],0,tmp,1,n,1);
		tmp = found_list_max(top[k]);
		if(fath[top[k]]) op[fath[top[k]]].insert(tmp);
		k=fath[top[k]];
	}
}
inline void del_upd(int k){
	while(k){
		pair<int,int> tmp = found_list_max(top[k]);
		if(fath[top[k]]) op[fath[top[k]]].erase(tmp);
		add(dfn[k],0,emp,1,n,1);
		k=fath[top[k]];
	}
}
inline void insert(int p,int x,int y){
	if(!p) del_upd(x);
	s[p][x].push(y);
	if(p==1) add(dfn[x],p,make_pair(found_val_max(p,x),x),1,n,1);
	else op[x].insert(make_pair(y,x)),add_upd(x);
}
inline void chose(int p,int x){
	...
	ANS += val,s[p][x].pop();
	if(!p){
		op[x].erase(make_pair(val,x));
		insert(1,x,-val),update(x,-1);
	}
	else{
		add(dfn[x],1,make_pair(found_val_max(p,x),x),1,n,1);
