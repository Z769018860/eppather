...
template <std::uint32_t ModT> class montgomery_modint30 {
...
public:
  ...
  constexpr montgomery_modint30(IntT v)
      : v_(redc(static_cast<u64>(v % SMOD + SMOD) * R2)) {}
  ...
  operator+(const montgomery_modint30 &lhs, const montgomery_modint30 &rhs) {
...
template <std::uint32_t ModT> using mm30 = montgomery_modint30<ModT>;




template <typename ModIntT> void zeta_transform(std::vector<ModIntT> &x) {
...
template <typename ModIntT> void moebius_transform(std::vector<ModIntT> &x) {
  const int n = static_cast<int>(x.size());
  assert((n & (n - 1)) == 0);
...
int main() {
  ...
  std::vector ranked_zeta_a(n + 1, std::vector<mint>(1 << n)),
      ranked_zeta_b(n + 1, std::vector<mint>(1 << n)),
      ranked_zeta_ab(n + 1, std::vector<mint>(1 << n));
  for (int i = 0; i != 1 << n; ++i) {
  ...
  for (int i = 0; i <= n; ++i)
    moebius_transform(ranked_zeta_ab[i]);
