...
namespace GCD
{
	...
	{
		int n=P.size(),m1=max(3*n/4,1);
		if(2*Q.size()<=n) return {0,{{1},{},{},{1}}};
		Poly P1(P.begin()+m1,P.end()),Q1(Q.begin()+m1,Q.end());
		auto [A,M1]=hgcd(P1,Q1);
		tie(P,Q)=M1.adj().apply(P,Q);
		if(2*Q.size()<=n) return {A,M1};
		auto [a,R]=DIV(P,Q);
		A++;
		swap(P,R);
		swap(sanitize(P),Q);
		if(2*Q.size()<=n) return {A,M1*Matrix{a,{1},{1},{}}};
		int m2=n-P.size();
...
int main()
{
	...
	if(~A&1) M.B=-M.B;
	M.B*=(ll)M.C.back()*power(G.back(),mod-2)%mod;
	M.B.resize(n);
