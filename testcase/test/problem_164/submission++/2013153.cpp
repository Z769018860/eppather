...
bigint &bigint::operator*=(const bigint &a){
	...
	if ((size <= 80 && a.size <= 80) || (size <= 8) || (a.size <= 8)){
		...
		if (size <= 240 && a.size <= 240){
			...
			u64 n = (max(size, a.size)+2)/3;
			bigint x0 = getitem(0, n);
			bigint x1 = getitem(n, n*2);
			bigint x2 = getitem(n*2, n*3);
			bigint y0 = a.getitem(0, n);
			bigint y1 = a.getitem(n, n*2);
			bigint y2 = a.getitem(n*2, n*3);
		    while (x0.size&&!x0.data[x0.size-1]) --x0.size;
			while (x1.size&&!x1.data[x1.size-1]) --x1.size;
			while (y0.size&&!y0.data[y0.size-1]) --y0.size;
			while (y1.size&&!y1.data[y1.size-1]) --y1.size;
		    bigint p0 = x0*y0;
		    bigint p1 = (x0+x1+x2)*(y0+y1+y2);
		    bigint p2 = (x0-x1+x2)*(y0-y1+y2);
		    bigint p3 = (x0+(x1<<1)+(x2<<2))*(y0+(y1<<1)+(y2<<2));
		    bigint p4 = (x0-(x1<<1)+(x2<<2))*(y0-(y1<<1)+(y2<<2));
		    bigint w0 = p0;
		    bigint w1 = ((((p1-p2)<<3)-p3+p4)>>2).div3();
		    bigint w2 = (((p0*-30)+((p1+p2)<<4)-p3-p4)>>3).div3();
		    bigint w3 = ((((p2-p1)<<1)+p3-p4)>>2).div3();
		    bigint w4 = ((((p0+p0+p0)<<1)-(p1<<2)-(p2<<2)+p3+p4)>>3).div3();
		    return ((((*this = w0) += (w1 <<= (n<<6))) += (w2 <<= (n<<7))) += (w3 <<= ((n*3)<<6))) += (w4 <<= (n<<8));
		}
	}
}
