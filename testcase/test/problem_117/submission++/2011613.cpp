...
const int N=55,M=3.5e5+5,inf=1e18;
struct node{
    int to,sm;
}p[M];
int idx=1,st,ed,dep[M],S,T,n,m,in[M],out[M],dbh[M],bh[M],ls[M],dys,res,now[M];
vector<int>e[M];
...
int dfs(int x,int sm){
    if(x==ed)return sm;
    int ans=sm,res=0;
    for(int i=now[x];i<e[x].size()&&ans;i++){
        now[x]=i;
        int v=e[x][i],c=p[v].to;
