...
void build_tree(ll arr[],ll tree[],ll node,ll start,ll end) 
{
    ...
    {
        ll mid=(start+end)/2;
        ll left_node = 2*node+1;          ll right_node =2 *node+2; 
        build_tree(arr,tree,left_node,start,mid);         build_tree(arr,tree,right_node,mid+1,end);        tree[node]=tree[left_node]+tree[right_node]; 
    }
}
