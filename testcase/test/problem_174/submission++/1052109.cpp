...
binomial_convolution_modulo_prime_power(const std::vector<std::uint32_t> &a,
  ...
  { 
    ...
    for (int i = 1; i < len; ++i) {
      ...
      fac[i] = u64(fac[i - 1]) * (i / p_pow[v[i] - v[i - 1]]) % pq;
  ...
  for (int i = 0; i != n; ++i) {
    ...
    x1[i] = a_hat * ip_pow1[v[i]], x2[i] = a_hat * ip_pow2[v[i]];
  }
  for (int i = 0; i != m; ++i) {
    u64 b_hat = u64(b[i]) * ifac[i] % pq;
    y1[i] = b_hat * ip_pow1[v[i]], y2[i] = b_hat * ip_pow2[v[i]];
  }
  int con_len = get_len(len);
  std::fill(x1 + n, x1 + con_len, mod1(0)), std::fill(y1 + m, y1 + con_len, mod1(0));
  std::fill(x2 + n, x2 + con_len, mod2(0)), std::fill(y2 + m, y2 + con_len, mod2(0));
...
binomial_convolution_modulo_composite(const std::vector<std::uint32_t> &a,
  ...
  std::function<void(u32, std::vector<u32> &, u32, const std::vector<u32> &)> crt =
      [](u32 m, std::vector<u32> &x, u32 m2, const std::vector<u32> &y) {
        int len = x.size();
