...
public:
  ...
  constexpr ModInt64(const m64 &rhs) : v(rhs.v) {}
  ...
  constexpr m64 &operator=(const m64 &rhs) { return v = rhs.v, *this; }
...
template <u64 P> std::pair<const ModInt64<P> *, const ModInt64<P> *> init(int n) {
  static int lim = 0;
...
template <u64 P> void dft(int n, ModInt64<P> x[], const ModInt64<P> *ROOT) {
...
template <u64 P> void idft(int n, ModInt64<P> x[], const ModInt64<P> *ROOT) {
  for (int i = 2; i < n; i <<= 1) {
    for (int j = 0, l = i >> 1; j != l; ++j) {
      ModInt64<P> u = x[j], v = x[j + l];
      x[j] = u + v, x[j + l] = u - v;
...
template <u64 P> void dft(int n, ModInt64<P> x[]) { dft(n, x, init<P>(n).first); }

template <u64 P> void idft(int n, ModInt64<P> x[]) { idft(n, x, init<P>(n).second); }

std::tuple<std::int64_t, std::int64_t, std::int64_t> exgcd(std::int64_t a, std::int64_t b) {
  std::int64_t x1 = 1, x2 = 0, x3 = 0, x4 = 1;
  while (b != 0) {
    std::int64_t q = a / b;
    std::tie(x1, x2, x3, x4, a, b) =
        std::make_tuple(x3, x4, x1 - x3 * q, x2 - x4 * q, b, a - b * q);
  }
  return std::make_tuple(a, x1, x2);
}

std::uint64_t inv_mod(std::uint64_t x, std::uint64_t y) { 
  std::int64_t iv = std::get<1>(exgcd(x, y));
  return iv < 0 ? iv + y : iv;
}
...
std::vector<u32> binomial_convolution_modulo_composite(const std::vector<u32> &a,
                                                       const std::vector<u32> &b, u32 p) {
  int m = 1, len = a.size() + b.size() - 1;
  std::vector<u32> res(len, 0);
  for (int i = 2; (long long)i * i <= p; ++i) {
    if (p % i == 0) {
      int p_old = p;
      while (p % i == 0) p /= i;
      u32 m2 = p_old / p, im1 = inv_mod(m, m2);
      std::vector<u32> resmodm2(binomial_convolution_modulo_prime_power(a, b, i, m2));
      for (int j = 0; j != len; ++j)
        res[j] = u64((resmodm2[j] + m2 - res[j] % m2) % m2) * im1 % m2 * m + res[j];
      m *= m2;
    }
  }
  if (p != 1) {
    u32 m2 = p, im1 = inv_mod(m, m2);
    std::vector<u32> resmodm2(binomial_convolution_modulo_prime_power(a, b, m2, m2));
    for (int j = 0; j != len; ++j)
      res[j] = u64((resmodm2[j] + m2 - res[j] % m2) % m2) * im1 % m2 * m + res[j];
    m *= m2;
  }
