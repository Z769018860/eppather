...
std::vector<int> convolution_mtt(const std::vector<int>& a,
...
std::vector<mint> inverse(const std::vector<mint>& a){
    ...
    std::vector<mint> f0 = inverse(std::vector<mint>(a.begin(), a.begin() + ((n + 1) >> 1)));
    int z = 1 << internal::ceil_pow2(2 * n);
    f0.resize(z);
    internal::butterfly(f0);
    std::vector<mint> b = a;
    b.resize(z);
    internal::butterfly(b);
    for(int i = 0; i < z; i++) f0[i] *= 2 - f0[i] * b[i];
    internal::butterfly_inv(f0);
    f0.resize(n);
    mint iz = mint(z).inv();
...
void Solve(int l, int r){
	...
	int mid = l + r >> 1;
	Solve(l, mid);
	auto C = convolution(vector<mint>(F + l, F + mid + 1), vector<mint>(G, G + r - l + 1));
	FL(i, mid + 1, r) F[i] += C[i - l];
	Solve(mid + 1, r);
}
int main(){
	scanf("%d%d", &n, &m);
	FL(i, 1, m){
		...
		if(x <= n) ++cnt[x];
		if(y && x * (y + 1ll) <= n) --cnt[x * (y + 1)];
	}
	FL(i, 1, n) for(int j = i; j <= n; j += i) G[j] += i * cnt[i];
	Solve(0, n);
	FL(i, 1, n) printf("%u\n", F[i].val());
    return 0;
}