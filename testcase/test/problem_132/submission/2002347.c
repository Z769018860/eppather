...
i64 sum(int u, int l, int r, int L, int R, i64 delta)
{
    if (L > r || R < l)
        return 0;
    if (l <= L && R <= r)
        return tr[u].sum + delta * (min(R, r) - max(L, l) + 1);
    int M = (L + R) >> 1;
    return sum(lc(u), l, r, L, M, delta + tr[u].tag_add) + sum(rc(u), l, r, M + 1, R, delta + tr[u].tag_add);
}
int main()
{
    n = rd(), q = rd();
