...
void dfs(int x)
{
	...
	for(int i=0;i<xw[x].size();i++)
	{
		...
		if(siz[qx[xw[x][i]]]>=qy[xw[x][i]])qans[xw[x][i]]=-1;
...
void dfs2(int x)
{
	...
	for(int i=0;i<v[x].size();i++)
	{
		...
		{
			nex[las[qx[pos[v[x][i]]]]]=fir[qy[pos[v[x][i]]]];
...
int main()
{
	...
	for(int i=1;i<=(n-1)/kk+1;i++)
	{
		...
		for(int j=1;j<=m;j++)
		{
			if(qans[j]==-1)xw[qz[j]].push_back(j);
		}
		for(int j=1;j<=n;j++)
		{
			fa[j]=j;
		...
		for(int j=1;j<=m;j++)
		{
			if(qans[j]==-2)xw[qz[j]].push_back(j);
		}
	}
	for(int i=1;i<=m;i++)
	{
