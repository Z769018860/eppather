...
vector<int>g[MAXN+5];
int f[MAXN+5],dep[MAXN+5],v[MAXN+5],res[MAXN+5];
...
void undo(){
	tuple<int,int,int>t=stk.top();stk.pop();
	int x=get<0>(t),y=get<1>(t),z=get<2>(t);
	f[y]=0;v[x]-=v[y];dep[x]-=z;
} 
void dfs1(int x,int bel){
	int tmp=stk.size();
	if(q[x].opt==1)merge(q[x].x,q[x].y);
	else if(q[x].opt==3&&!res[x]){
		int val=v[find(q[x].x)];
...
int main(){
	...
	a[0]=-1;blk_sz=2000;blk_cnt=(n-1)/blk_sz+1;
	...
	for(int i=1;i<=blk_cnt;i++){
		...
		memset(v,0,sizeof(v));while(!stk.empty())stk.pop();
