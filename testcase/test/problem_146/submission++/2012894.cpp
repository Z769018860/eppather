...
int main()
{
    ...
    std::function<void(int, int, int)> bit_add = [&](int k, int x, int dep)
    {
        bit_1.add(k, x), bit_2.add(k, 1ll * dep * x);
    };
    std::function<i64(int, int)> bit_sum = [&](int k, int delta)
    {
        return bit_2.sum(k) - bit_1.sum(k) * delta;
    };
    ...
    for (int i = 1; i <= n; ++i)
    {
        int u = lca.rev_dfn[i], f = lca.f[u];
        bit_1.a[i] += a[u];
        bit_2.a[i] += 1ll * a[u] * lca.dep[u];
        bit_1.a[lca.dfn[f]] -= a[u];
        bit_2.a[lca.dfn[f]] -= 1ll * a[u] * lca.dep[f];
    }
    ...
    while (q--)
    {
        int op = FastIO::rd(), u = FastIO::rd(), v = 0, w = 0, x = 0, y = 0;
        switch (op)
        {
        case 1:
            v = FastIO::rd(), w = FastIO::rd();
            x = lca.lca(u, v), y = lca.f[x];
            bit_add(lca.dfn[u], w, lca.dep[u]), bit_add(lca.dfn[v], w, lca.dep[v]);
            bit_add(lca.dfn[x], -w, lca.dep[x]), bit_add(lca.dfn[y], -w, lca.dep[y]);
            break;
        case 2:
            FastIO::wr(bit_1.sum(lca.dfn[u] + lca.sz[u] - 1) - bit_1.sum(lca.dfn[u] - 1));
            break;
        case 3:
            FastIO::wr(bit_sum(lca.dfn[u] + lca.sz[u] - 1, lca.dep[u] - 1) - bit_sum(lca.dfn[u] - 1, lca.dep[u] - 1));
            break;
        default:
            break;
        }
