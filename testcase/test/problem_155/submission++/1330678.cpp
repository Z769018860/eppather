...
template <typename T>
inline void ranked_zeta_trans(const std::vector<T> &f, T ret[][21]) {
  const int n = f.size();
...
template <typename T>
inline std::vector<T> ranked_moebius_trans(T mat[][21], const int n) {
...
inline std::vector<T> subset_convolution(std::vector<T> f, std::vector<T> g) {
  ...
  if (D <= 10) return subset_conv_naive(f, g);
  assert(__builtin_popcount(n) == 1 && f.size() == g.size());
  static T F[1 << 21][21], G[1 << 21][21], tmp[21];
  for (int S = n, c; S--;)
    c = __builtin_popcount(S), std::fill_n(F[S], c, 0), std::fill_n(G[S], c, 0);
  ranked_zeta_trans(f, F), ranked_zeta_trans(g, G);
  ...
  return ranked_moebius_trans(F, n);
}
