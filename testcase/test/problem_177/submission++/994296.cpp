...
public:
    ...
    explicit operator long long () {
        return this->to_long_long();
    }
    operator void *() {
        return this->is_zero() ? 0 : this;
    }
private:
...
void Decimal::init() {
    is_neg = false, s.clear(), s.push_back(0);
}
...
Decimal::Decimal(const Decimal &b) {
    is_neg = b.is_neg, s = b.s;
}

Decimal::Decimal(const vector &digits, bool is_negative) {
    is_neg = is_negative, s = digits, this->canonicity();
}

bool Decimal::is_zero() const {
    return s.size() == 1 && s.back() == 0;
}
...
long long Decimal::to_long_long() const {
    ...
    return is_neg ? (v > LLONG_MAX ? LLONG_MIN : -(long long)v) : (v >= LLONG_MAX ? (long long)LLONG_MAX :
            (long long)v);
}
...
int Decimal::compare_with(int x) const {
...
Decimal &Decimal::operator = (const string &s) {
    return this->initstr(s.c_str()), *this;
}

Decimal &Decimal::operator = (const char *s) {
    return this->initstr(s), *this;
}
...
Decimal &Decimal::abs() {
    return is_neg = false, *this;
}

Decimal &Decimal::neg() {
    return is_neg ^= !this->is_zero(), *this;
}
...
std::ostream &operator << (std::ostream &out, const Decimal &b) {
...
void __builtin_abs_add_long_long(vector &s, long long x) {
    ...
    for (i = 0; i < n; ++i)
        if (s[i] >= Decimal::BASE)
            s[i + 1] += s[i] / Decimal::BASE, s[i] %= Decimal::BASE;
}
...
void __builtin_abs_sub_long_long(vector &s, long long x) {
    ...
    for (i = 0; i < n - 1; ++i)
        if (s[i] < 0)
            s[i + 1] += d = (s[i] + 1) / Decimal::BASE - 1, s[i] -= d * Decimal::BASE;
}
...
Decimal &Decimal::operator += (long long x) {
    ...
    if (x == LLONG_MIN)
        return *this -= Decimal("9223372036854775808");
    ...
    if (!is_neg)
        ...
        case 1 :
            return *this = Decimal(this->to_long_long() + x);
        }
...
Decimal operator + (const Decimal &a, const Decimal &b) {
    if (a.is_neg == b.is_neg) {
        ...
        for (i = 0; i < min; ++i) {
            ...
            if (ret.s[i] >= Decimal::BASE)
                ret.s[i] -= Decimal::BASE, ++ret.s[i + 1];
        }
...
#else
        guess_quo = (((long double)a.s.back() * Decimal::BASE + a.s[n - 2]) * Decimal::BASE + a.s[n - 3] + 1) / B,
        guess_quo += 2;
#endif
    else
...
inline void mul(cperm g, cperm h, pperm ret, int n = L) {
    for (int i = 0; i < n; ++i)
        ret[i] = g[h[i]];
}
inline void inv(cperm g, pperm ret, int n = L) {
    for (int i = 0; i < n; ++i)
        ret[g[i]] = i;
}
inline void mulinv(cperm g, cperm h, pperm ret, int n = L) {
    for (int i = 0; i < n; ++i)
        ret[h[i]] = g[i];
}
inline void read(pperm g, int n = L) {
    for (int i = 0, x; i < n; ++i)
        cin >> x, g[i] = x - 1;
}
}
...
struct schreier_sims {
    ...
    inline int index() const {
        return std::count(flag, flag + size, true);
    }
    ...
    inline void update_transversal(cperm g) {
        ...
        if (flag[pos]) {
            if (succ)
                mul(transversal[pos], g, h, size), succ->update_generator(h);
        } else {
            flag[pos] = true, inv(g, transversal[pos], size);

            for (int i = 0; i < n_gen; ++i)
                mul(generator[i], g, h, size), update_transversal(h);
        }
    }
    ...
    inline void update_generator(cperm g) {
        ...
        if (!test(g)) {
            ...
            for (int i = 0; i < size; ++i)
                if (flag[i])
                    mulinv(g, transversal[i], h, size), update_transversal(h);
        }
    }
} ss[N];
...
void work() {
    ...
    for (i = 0; i < L; ++i)
        ss[i].reset(i + 1, i ? ss + (i - 1) : NULL);

    for (i = 0; i < n; ++i)
        read(x), ss[L - 1].update_generator(x);
