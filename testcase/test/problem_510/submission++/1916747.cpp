...
struct node{ll l,r,s;}tr[N<<6];
vector<ll> cir[N];
ll n,q,k,i,j,op,x,y,vis[N],tot,in_head[N],in_circle[N],jw[N],las,cnt,head_cir[N],id,now,head,root[N],tr_tot,jw_cir[N],ans,temp,dep,qmi[N],id_dep[N],num[N],numz,len[N];
ll ttt,ttt2,ttt3,numm,depp;
unordered_map<ll,ll> calc;
unordered_map<ll,ll> get_id[35];
unordered_map<ll,ll> get_id2[N];
...
inline ll query(ll x,ll s,ll t,ll p){
	if(!p) return 0;
	if(s==t) return tr[p].s;
	if(x>(s+t)/2) return tr[tr[p].l].s^query(x,(s+t)/2+1,t,tr[p].r);
	else return query(x,s,(s+t)/2,tr[p].l);
}
int main(){
	...
	while(q--){
		...
		if(op==1){
			...
			if(x<=1e9){
				...
				while(ttt--) x/=k;
				x /= k;
				if(!get_id[ttt2][dep*(1e9+1)+x%jw[dep]]) get_id[ttt2][dep*(1e9+1)+x%jw[dep]]=++id;
				now = get_id[ttt2][dep*(1e9+1)+x%jw[dep]];
			...
			while(x){
				...
				dep = id_dep[temp];
				if(in_circle[temp]){
					ans ^= query(num[temp],0,len[dep],root[get_id2[dep][(x-ttt3+cir[id_dep[temp]][0]*qmi[ttt]-jw_cir[temp]*qmi[ttt+1])]]);
					head = (x-ttt3-jw_cir[temp]*qmi[ttt+1]);
					if(head>=0){
						head /= numm*k;
						ans ^= query(head,0,1e9,root[get_id[ttt2][dep*(1e9+1)+head%jw[dep]]]);
					}
				}
				ans ^= calc[x];
				x -= ttt3;
