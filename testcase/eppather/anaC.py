...
# 定义函数：检查是否有指针定义
def contains_pointer(code):
    pointer_pattern = r'\*([a-zA-Z_][a-zA-Z0-9_]*)'
    return bool(re.search(pointer_pattern, code))

# 定义函数：检查是否有结构体定义
def contains_struct(code):
    struct_pattern = r'\bstruct\s+[a-zA-Z_][a-zA-Z0-9_]*\s*\{'
    return bool(re.search(struct_pattern, code))
...
def extract_function_names(code):
    ...
    names = [re.match(r'\b[a-zA-Z_][a-zA-Z0-9_]*\s+([a-zA-Z_][a-zA-Z0-9_]*)', func).group(1) for func in functions]
    return names
...
def process_c_files_and_export_to_excel(folder_path, output_excel):
    ...
    headers = [
        ...
        "Contains Pointer", "Contains Struct", "Contains Nested Structure", 
        "Contains Bitwise Operations", "Function Count", "Function Names", 
        "Function Parameters"
    ]
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.c'):  # 只处理 .c 文件
            ...
            has_pointer = contains_pointer(code)
            has_struct = contains_struct(code)
            ...
            sheet.cell(row=row, column=5, value="Yes" if has_pointer else "No")
            sheet.cell(row=row, column=6, value="Yes" if has_struct else "No")
            sheet.cell(row=row, column=7, value="Yes" if nested_structure else "No")
            sheet.cell(row=row, column=8, value="Yes" if bitwise_operations else "No")
            sheet.cell(row=row, column=9, value=function_count)
            sheet.cell(row=row, column=10, value=", ".join(function_names))
            sheet.cell(row=row, column=11, value=", ".join(map(str, param_counts)))
            row += 1
