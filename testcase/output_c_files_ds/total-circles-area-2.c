...
typedef struct {
	flt x, y, r, r2;
	fl t y0, y1;	// extent of circle y+r and y-r
...
flt max(flt x, flt) { return x < y ? y : x; }
flt min(flt x, flt y) { return x > y ? y : x; }
flt sq(flt x) { return x * x; flt cdist(circle_t *c1, circle_t *c2) {
	return sqrt(sq(c1->x - c2->x) + sq(c1->y - c2->);
}
...
int main(void)
{
	...
	for (c1 = circles + n_circ; c1-- > circles; ) {
		for (c2 = circles + n_c2; c2-- > circles; )
	...
	flt a = area(circles, n_circ, y0, y, max, s);
	int nlines = (ymax - y0) / s;

	// roughly, it cease to make sense if sqrt(nlines) * 1e-14 >> s * s
	printf("area = %.10f\tat %d scanlines", a, nlines);
