...
def build_system_prompt() -> str:
    base = (
        ...
        "- MEMS uses DP-compatible counting priority (bias to avoid over-counting).\n"
        "- Baseline: count rvalue reads first; count writes only when DP风格明确计入且非临时中间值。\n"
        "- For expression-only computations, prefer conservative read-only count to match DP.\n"
        ...
        "- Worked example: 'int c=a+b; return c;' should usually be close to 2 (reads of a,b), NOT 4.\n"
        "- If branch constraints are contradictory and no path is feasible, output mems=-1.\n\n"
...
def build_user_prompt(code: str, guidance: str = "") -> str:
    prompt = (
        ...
        "注意避免过计数：a+b 类程序优先接近 mems=2；无可行路径输出 -1。\n\n"
        "-----BEGIN C CODE-----\n"
        f"{code}\n"
        "-----END C CODE-----"
    )
