...
DP_PREFIX_BUDGET_SKIPS_RE = re.compile(r"(?m)^\[DP PREFIX BUDGET SKIPS\]:\s*(\d+)")
...
def parse_dp_blocks(text: str) -> list[dict]:
    ...
    for part in parts[1:]:
        ...
        prefix_budget_skips_m = DP_PREFIX_BUDGET_SKIPS_RE.search(rest)
        ...
        blocks.append({
            ...
            "prefix_budget_skips": int(prefix_budget_skips_m.group(1)) if prefix_budget_skips_m else "",
    ...
    with tempfile.TemporaryDirectory(prefix="eppather-maxmems266-") as td:
        ...
        for block in blocks:
            ...
            row = {
                ...
                "dp_prefix_budget_skips": block.get("prefix_budget_skips", ""),
