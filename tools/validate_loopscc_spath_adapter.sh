...
if ! grep -Eq '^\[LOOPSCC CYCLE\]: .*phase_guards_proved=1 .*closed_form_candidate=1
if [[ "$periodic_trip_count" != 4 ]]; then
  echo "periodic: expected LoopSCC to prove four iterations despite --maxloop 1" >&2
  cat "$OUT_DIR/periodic.log" >&2
  exit 1
...
if ! grep -Eq '^\[LOOPSCC ACCELERATION PLAN\]: .*mems_preserving=1 skippable_iterations=4 exact=1
if [[ "$periodic_trip_count" != 4 ]]; then
  echo "periodic: expected LoopSCC to prove four iterations despite --maxloop 1" >&2
  cat "$OUT_DIR/periodic.log" >&2
  exit 1
