...
printf '%s\n' \
  'id,category,features,maxloop,mode,compile_status,run_status,path_count,solution_space_count,weighted_average_mems,dfs_max_mems,canonical_memory_regions,memory_projection_arity,memory_projection_status,max_bounded_memory_terms,max_factored_projection_components,zero_diagnostic' \
  > "$OUT_DIR/summary.csv"
...
while IFS=',' read -r id source category features maxloop; do
  ...
  for mode in scalar memory; do
    ...
    max_terms="$(sed -n 's/^\[VOLCE BOUNDED MEMORY TERMS\]: //p' "$log" | \
      sort -nr | head -1)"
    max_factored="$(sed -n 's/^\[VOLCE FACTORED PROJECTION COMPONENTS\]: //p' "$log" | \
      sort -nr | head -1)"
    paths="${paths:-0}"
    count="${count:-N/A}"
    average="${average:-N/A}"
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      ...
      "$max_factored" "$zero_diagnostic" >> "$OUT_DIR/summary.csv"
  done
