...
def main():
    ...
    summary={'problems_total':len(rows),'status_ok':len(ok),'validated_applicable':len(applicable),
      ...
      'effective_maxloop_counts':dict(sorted(collections.Counter(str(r.get('effective_maxloop')) for r in applicable).items()))}
    ...
    fields=['problem_id','status','effective_maxloop','branches','candidate_paths','validated_candidates','minimum_tests','eppather_predicted_edges','eppather_predicted_coverage','eppather_covered_edges','eppather_coverage','official_tests','official_covered_edges','official_coverage','coverage_prediction_correct','path_replay_matches','detail']
    ...
    for r in rows: md.append(f"|{r['problem_id']}|{r['status']} (L={r.get('effective_maxloop','-')})|{r.get('candidate_paths','')}|{r.get('minimum_tests','')}|{r.get('eppather_coverage','')}|{r.get('official_tests','')}|{r.get('official_coverage','')}|{r.get('coverage_prediction_correct','')}|")
