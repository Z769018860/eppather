...
ll ttt,ttt2,ttt3,depp;
ll qmi[N],x,y,head,numm,u;
...
int main(){
	...
	while(q--){
		...
		if(op==1){
			x=read(),y=read();
			numm = 1;
			while(x%(numm*k)==0) numm*=k;
			temp = (x/numm)%k,ttt = numm*temp;
			while(!in_circle[temp]&&x<=1e9){
				calc[x]^=y,x+=ttt;
				while(x%(numm*k)==0) numm*=k;
				temp = (x/numm)%k,ttt = numm*temp;
			}
			...
			u = (x-ttt3+cir[id_dep[temp]][0]*qmi[ttt]-jw_cir[temp]*qmi[ttt+1]);
			...
			while(x){
				...
				dep = id_dep[temp];
				if(in_circle[temp]){
					ans ^= query(0,num[temp],0,len[dep],root[get_id2[dep][(x-ttt3+cir[id_dep[temp]][0]*qmi[ttt]-jw_cir[temp]*qmi[ttt+1])]]);
					head = (x-ttt3-jw_cir[temp]*qmi[ttt+1]);
					if(head>=0){
						head /= numm*k;
						ans ^= query(0,head,0,1e9,root[get_id[ttt2][dep*(1e9+1)+head%jw[dep]]]);
					}
				}
				ans ^= calc[x];
				x -= ttt3;
