...
long long qmi[N],x,y,head,numm,u;
...
inline void query(ll l,ll r,ll s,ll t,ll p){
	...
	if(l<=(s+t)/2) query(l,r,s,(s+t)/2,tr[p].l);
	if(r>(s+t)/2) query(l,r,(s+t)/2+1,t,tr[p].r);
}
int main(){
	...
	while(q--){
		...
		if(op==1){
			...
			u = (1ll*x-ttt3+1ll*cir[id_dep[temp]][0]*qmi[ttt]-1ll*jw_cir[temp]*qmi[ttt+1]);
			...
			while(x){
				...
				dep = id_dep[temp];
				if(in_circle[temp]){
					query(0,num[temp],0,len[dep],root[get_id2[dep][1ll*(x-ttt3+1ll*cir[id_dep[temp]][0]*qmi[ttt]-1ll*jw_cir[temp]*qmi[ttt+1])]]);
					head = (x-ttt3-1ll*jw_cir[temp]*qmi[ttt+1]);
					if(head>=0){
						head /= numm*k;
						query(0,head,0,1e9,root[get_id[ttt2][1ll*dep*(1e9+1)+head%jw[dep]]]);
					}
				}
				ans ^= calc[x];
				x -= ttt3;
