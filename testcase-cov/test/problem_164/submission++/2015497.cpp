...
friend inline bigint operator<<(const bigint&, const u64);
friend inline bigint operator>>(const bigint&, const u64);
...
friend inline bool operator<=(const bigint&, const bigint&);
friend inline bool operator>=(const bigint&, const bigint&);
friend inline ostream &operator<<(ostream&, const bigint&);
friend inline istream &operator>>(istream&, bigint&);

friend bigint pow(bigint, u64);
...
private:
	...
	bigint(u64 *data, u64 size);
	bigint(bool sign, u64 *data, u64 size);
	void resize(u64, u64 = 0);
	void push(u64 n);
	void clear();
	void change_sign();
	bigint &change();
	bigint &make(const string&);
};
...
bigint::bigint(u64 *data, u64 size): data(copy_new(data, size)), size(size), capacity(size){}
...
bigint &bigint::operator*=(const bigint &a){
	...
	if ((size <= 80 && a.size <= 80) || (size <= 8) || (a.size <= 8)){
		...
		if (n >= size) return *this = ((*this*bigint(a.data+n, a.size-n))<<(n<<6))+(*this*bigint(a.data, n));
		if (n >= a.size) return *this = ((a*bigint(data+n, size-n))<<(n<<6))+(a*bigint(data, n));
		bigint l(data+n, size-n), r(data, n);
		bigint l2(a.data+n, a.size-n), r2(a.data, n);
