...
using namespace std;
int n,k,bn,num;
unsigned int r[89449],f[89449],g[89449],s[89449],phi[89449],phi1[89449],ans;
bool vis[44727];
...
int main(){
  ...
  for(int j=2;j<=bn;j++){
    ...
    for(int i=num;r[i]>=j*j;i--)g[i]-=g[id(r[i]/j)]-g[j-1],phi1[i]-=j*(phi1[id(r[i]/j)]-phi1[j-1]);
  }
  ...
  for(int j=bn;j>=2;j--){
    ...
    for(int i=num;r[i]>=j*j;i--){
      long long p=j;
      for(int e=1;p*j<=r[i];e++,p*=j){
        s[i]+=s[id(r[i]/p)]+pk*(g[id(r[i]/p)]-g[j-1]);
        phi[i]+=(p-p/j)*(phi[id(r[i]/p)]-phi1[j])+(p*j-p);
      }
    }
  }
