...
void shiftSample(vector<Mint> ys, const Mint &a, Mint *zs) {
  const int n = ys.size();
  if (n == 0) return;
  for (int i = 0; i < n; ++i) ys[i] *= invFac[i];
  vector<Mint> coef(n);
  for (int i = 0; i < n; ++i) coef[i] = (i & 1) ? -invFac[i] : invFac[i];
  ys = convolve(ys, coef);
  ys.resize(n);
  for (int i = 0; i < n; ++i) ys[i] *= fac[i];
  
  vector<Mint> as(n);
  as[n - 1] = 1;
  for (int i = 0; i < n - 1; ++i) as[n - 1 - (i + 1)] = as[n - 1 - i] * (a - i) * inv[1 + i];
  ys = convolve(ys, as);
  ys.erase(ys.begin(), ys.begin() + (n - 1));
  
  for (int i = 0; i < n; ++i) ys[i] *= invFac[i];
  for (int i = 0; i < n; ++i) coef[i] = invFac[i];
  ys = convolve(ys, coef);
  for (int i = 0; i < n; ++i) zs[i] = fac[i] * ys[i];
}
vector<Mint> shiftSample(const vector<Mint> &ys, const Mint &a) {
  vector<Mint> zs(ys.size());
  shiftSample(ys, a, zs.data());
  return zs;
}
...
int main() {
  ...
  for (; ~scanf("%d%d", &N, &M); ) {
    ...
    const vector<Mint> ans = shiftSample(Y, M);
    for (int i = 0; i <= N; ++i) {
      if (i) printf(" ");
      printf("%u", ans[i].x);
    }
