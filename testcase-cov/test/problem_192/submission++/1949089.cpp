...
struct hashfunc {
    ...
    int operator()(pll x) const { return hash_f(x.ff) ^ hash_f(x.ss); }
};
lld ttl = 0;
random_device rd;
mt19937 gen(7122); 
...
lld p = 0;
const pll O = {-1, -1};
inline lld M(lld x){ return x >= p? x-p:x; }
...
inline pll sum(pll x, pll y, lld a, lld b){
  if(x == O || y == O) return max(x, y);
  if(x.ff == y.ff && (x.ss + y.ss == 0 || x.ss + y.ss == p)) return O;
  ttl++;
  lld lam;
  if(x == y)
...
lld try_solve(lld a, lld b){
  ...
  for(lld i = 0; i < step; i++){
    if(mps.find(cur) != mps.end()){
      ...
      if(mps[cur] == -1)throw -1;
      if(mps[cur] + i > r);
      else if(match == -1)match = mps[cur] + i;
      else throw -1;
    }
    cur = sum(cur, x, a, b);
  }
  ...
  while(t--){
    ...
    if(p<=500){
      ...
      for(int i = 0; i < p; i++){
        ...
        if(!r)ans++;
        else if(pow(r, (p-1)/2) == 1)ans += (p!=2)+1;
      }
    ...
    if(M(modmul(modmul(a<<2, a), a)+modmul(9*b, 3*b)) == 0){
      if(!a && !b)
        ...
        if(pow(t, (p-1)/2) == 1) cout << (int64_t)p << endl;
        else cout << (int64_t)p+2 << endl;
      }
      continue;
    }
    ...
    for(int i = 1; true; i = !i){
      try{
        if(i) cout << (int64_t)try_solve(a, b) << endl;
        else cout << (int64_t)(2*p+2-try_solve(aa, bb)) << endl;
        break;
      }catch(int32_t e){
        fail++;
        if(fail > 2000) {
