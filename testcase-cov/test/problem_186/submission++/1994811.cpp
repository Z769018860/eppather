...
class LinkCutTree
{
	...
	public:
		...
		inline unsigned int find(unsigned int x){
			access(x),splay(x);
			while(node[x].child[0])pushdown(x),x=node[x].child[0];
			return splay(x),x;
		}
		inline void split(unsigned int x,unsigned int y){makeroot(x),access(y),splay(y);}
		inline void link(unsigned int x,unsigned int y){if(find(x)!=find(y))makeroot(x),node[x].parent=y;}
		inline void cut(unsigned int x,unsigned int y){
			makeroot(x);
			if(x==find(y)&&x==node[y].parent&&!node[y].child[0])node[y].parent=node[x].child[1]=0,pushup(x);
		}
};
int main()
{
