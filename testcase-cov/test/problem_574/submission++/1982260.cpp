...
inline void add(int x,int y,ppp c,int s,int t,int p){
	if(s==t){
		pai[y][p] = c;
		return ;
	}
...
inline void insert(int p,int x,int y){
	if(!p) del_upd(x);
	s[p][x].push(y);
	if(p==1) add(dfn[x],p,(ppp){found_val_max(p,x),x},1,n,1);
...
inline void chose(int p,int x){
	...
	if(!p){
		...
		add(dfn[x],1,(ppp){found_val_max(p,x),x},1,n,1);
