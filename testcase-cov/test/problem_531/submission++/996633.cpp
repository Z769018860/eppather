...
namespace Fast_IO{
	...
	char read_char() {
		return csy1 == csy2 && (csy2 = (csy1 = BUF_R) + fread(BUF_R, 1, BUF_SIZE, stdin), csy1 == csy2) ? EOF : *csy1 ++;
	}
	template <class T>
	void read_int(T &t) {
		int u = 0;
		...
		for (; c > 47 && c < 58; c = read_char()) t = (t << 1) + (t << 3) + (c ^ 48);
		t = u ? -t : t;
	}
	...
	void write_int(T x) {
	    ...
	    while (top) write_char(sta[-- top] ^ 48);
	}
	void flush() {
        if (csy - BUF_W > 0) fwrite(BUF_W, 1, csy - BUF_W, stdout);
        csy = BUF_W;
    }
}
...
void dfs(int u) {
    ...
    for (int v = son[u]; v; v = bro[v])
        if (v > 1) {
            dfs(v);
            if (circle[v]) circle[u] = 1;
            else if (!sg[v]) sg[u] = 1;
        }else {
            tot = dep[u];
            circle[u] = 1;
        }
    if (circle[u]) node[dep[u]] = u;
}
...
int main() {
    ...
    dfs(1);
    for (int i = tot; i > 0; i --) closest[i] = sg[node[i]] ? i : closest[i + 1];
    while (q --) {
        int u, v, c;
        ...
        if (u == 1 || dep[v] < dep[u]) {
            ...
            else write_char(50), write_char('\n');
        }else if (c = find(1, dep[fa[u]])) {
            write_char(((c & 1) ^ 48)), write_char('\n');
        }else write_char(((dep[u] & 1) ^ 48)), write_char('\n');
    }
    flush();
    Please AC;
}