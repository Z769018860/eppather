...
poly solve_equal_degree_equation(const poly &x) { 
  int d = deg(x);
  if (d <= 0) return {};
  if (d == 1) return {-x[0] / x[1]};
  poly y = random_poly(d - 1), z = gcd(x, y);
  if (deg(z) <= 0)
    z = gcd(x, pow_mod(y, ModInt32::get_mod() - 1 >> 1, x) + poly{1}); 
  poly x1 = solve_equal_degree_equation(x / z), x2 = solve_equal_degree_equation(z);
...
int main() {
  ...
  std::cout << cnt << '\n';
  if (cnt != 0) {
    auto ans = solve_equal_degree_equation(b);
    std::vector<int> res;
