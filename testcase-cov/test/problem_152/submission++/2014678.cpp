...
int main() {
  ...
  std::vector<mint> f10_4(1 << n);
  if (n >= 10) {
    for (int i = 0; i != 1 << n; ++i) f10_4[i] = ranked_zeta_ab[10][i] + ranked_zeta_ab[4][i];
    moebius_transform(f10_4);
  }
  for (int i = 0; i <= n; ++i) moebius_transform(ranked_zeta_ab[i]);
  for (int i = 0; i != 1 << n; ++i) {
    const auto c = std::popcount(static_cast<unsigned>(i));
    std::cout << (n >= 10 && (c == 10 || c == 4) ? f10_4[i] : ranked_zeta_ab[c][i]) << ' ';
  }
}
