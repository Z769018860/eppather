...
struct ntt {
    ...
    void trans(int l, vector<LMod> &A) const {
        ...
        for (int e = l & ~1; e >= 2; e -= 2) {
            ...
            for (size_t i = 0; i < n; i += m) {
                const LMod w1 = w2 * w2, w3 = w1 * w2; 

                
                for (size_t j = i; j < i + m4; ++j) {
                    LMod a0 = A[j + m4 * 0] * one, a1 = A[j + m4 * 1] * w2; 
                    LMod a2 = A[j + m4 * 2] * w1,  a3 = A[j + m4 * 3] * w3; 
                    LMod t02p = a0 + a2, t13p = a1 + a3; 
                    LMod t02m = a0 - a2, t13m = (a1 - a3) * imag; 
                    A[j + m4 * 0] = t02p + t13p;
                    A[j + m4 * 1] = t02p - t13p; 
                    A[j + m4 * 2] = t02m + t13m;
                    A[j + m4 * 3] = t02m - t13m; 
        ...
        for (int e = 2; e <= l; e += 2) {
            ...
            for (size_t i = 0; i < n; i += m) {
                ...
                for (size_t j = i; j < i + m4; ++j) {
                    ...
                    LMod t01p = a0 + a1, t23p = a2 + a3; 
                    LMod t01m = a0 - a1, t23m = (a2 - a3) * imag; 
    ...
    template <typename T>
    vector<u64> convolve_binom(
        const vector<T> &f, const vector<T> &g, int p, const std::vector<int> &vs) const {
...
struct ExactDiv {
    ExactDiv() {}
    constexpr ExactDiv(u64 n) : mod(n), shift(__builtin_ctzll(n)),
        mask((u64(1) << shift) - 1), inv(mul_inv(n >> shift)) {}
...
struct BinConvPre {
    BinConvPre(int n, int p, int pe)
        : facts(n + 1), ifacts(n + 1), vs(n + 1) {
