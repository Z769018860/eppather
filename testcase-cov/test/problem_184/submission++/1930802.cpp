...
vi unit_monge_mul_impl(const vi& a, const vi& rb) {
  int n = a.size();
  if (n == 1) return a;
  int m = n / 2;
  ...
  vi cl = unit_monge_mul_impl(al, bl);
  vi cr = unit_monge_mul_impl(ar, br);
...
vi unit_monge_mul(const vi& a, const vi& b) {
  int n = a.size();
  vi rb(n);
  for (int i = 0; i < n; i++) rb[b[i]] = i;
  return unit_monge_mul_impl(a, rb);
}
