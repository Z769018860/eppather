...
template <typename T>
inline void ranked_zeta_trans(const T f[], T ret[][MAX_N], const int &n) {
  for (int S = n; S--;) ret[S][__builtin_popcount(S)] = f[S];
  SUBSET_REP(S, U, n)
  for (int d = __builtin_popcount(S); d--;) ret[S][d] += ret[U][d];
}

template <typename T>
inline void ranked_moebius_trans(T mat[][MAX_N], T ret[], const int &n) {
  const int D = __builtin_ctz(n);
  SUBSET_REP(S, U, n)
...
template <typename T>
inline void subset_conv_naive(const T f[], const T g[], T ret[], const int &n) {
  for (int s = n, t; s--;)
    for (ret[t = s] = f[0] * g[s]; t; (--t) &= s) ret[s] += f[t] * g[s ^ t];
}

template <typename T>
inline void subset_conv_cont(const T f[], const T g[], T ret[], const int &n) {
  static T F[1 << MAX_N][MAX_N], G[1 << MAX_N][MAX_N], tmp[MAX_N];
  for (int S = n, c; S--;)
    c = __builtin_popcount(S), std::fill_n(F[S], c, 0), std::fill_n(G[S], c, 0);
  ranked_zeta_trans(f, F, n), ranked_zeta_trans(g, G, n);
  const int D = __builtin_ctz(n);
  for (int S = n, c, d, e, bg; S--;) {
    c = __builtin_popcount(S), bg = std::min(2 * c, D);
    for (d = bg; d >= c; d--)
      for (tmp[d] = 0, e = d - c; e <= c; e++) tmp[d] += F[S][e] * G[S][d - e];
    for (d = bg; d >= c; d--) F[S][d] = tmp[d];
  }
  ranked_moebius_trans(F, ret, n);
}
...
inline std::vector<T> subset_convolution(std::vector<T> f, std::vector<T> g) {
  ...
  std::vector<T> ret(n);
  if (D <= 10) return subset_conv_naive(f.data(), g.data(), ret.data(), n), ret;
  assert(n == 1 << D && f.size() == g.size());
  return subset_conv_cont(f.data(), g.data(), ret.data(), n), ret;
}
...
inline std::vector<T> subset_exp(const std::vector<T> &f) {
  ...
  for (; l < ed; l <<= 1)
    subset_conv_naive(ret.data(), f.data() + l, ret.data() + l, l);
  if (l < sz) {
    for (int S = sz; S--;) std::fill_n(F[S], __builtin_popcount(S), 0);
    for (int S = l >> 1; S--;) F[S][__builtin_popcount(S)] = ret[S];
    SUBSET_REP(S, U, (l >> 1))
    for (int d = __builtin_popcount(S); d--;) F[S][d] += F[U][d];

    for (int D = __builtin_ctz(l); l < sz; l <<= 1, D++) {
      for (int S = (l >> 1); S < l; S++) F[S][__builtin_popcount(S)] = ret[S];

      for (int _ = 1; _ < (l >> 1); _ <<= 1)
        for (int __ = l >> 1, _2 = _ << 1; __ < l; __ += _2)
          for (int U = __, S = U | _, ___ = S; U < ___; U++, S++)
            for (int d = __builtin_popcount(S); d--;) F[S][d] += F[U][d];

      for (int U = 0, S = l >> 1; S < l; U++, S++)
        for (int d = __builtin_popcount(S); d--;) F[S][d] += F[U][d];

      for (int S = l; S--;) std::fill_n(G[S], __builtin_popcount(S), 0);
      ranked_zeta_trans(f.data() + l, G, l);
      for (int S = l, c, d, e, bg; S--;) {
        c = __builtin_popcount(S), bg = std::min(2 * c, D);
        for (d = bg; d >= c; d--)
          for (tmp[d] = 0, e = d - c; e <= c; e++)
            tmp[d] += F[S][e] * G[S][d - e];
        for (d = bg; d >= c; d--) G[S][d] = tmp[d];
      }
      SUBSET_REP(S, U, l)
      for (int c = __builtin_popcount(S), d = std::min(2 * c, D); d >= c; d--)
        G[S][d] -= G[U][d];
      for (int S = l; S--;) ret[S + l] = G[S][__builtin_popcount(S)];
    }
  }
  return ret;
}
...
T tutte_polynomial(const std::vector<std::vector<int>> &adj, T x, T y) {
  ...
  static int sum[1 << 21], s, ed, t, used[21] = {0}, lim = 2, i, j;
