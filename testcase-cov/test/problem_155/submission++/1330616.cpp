...
inline std::vector<T> subset_composite(const std::vector<T> &f,
  ...
  for (int k = 1, l = 1, l2 = 2; k <= m; k++, l <<= 1, l2 = l << 1)
    for (int j = 0; j <= m - k; j++) {
      auto c = subset_convolution(h[j + 1], {f.begin() + l, f.begin() + l2});
      h[j].insert(h[j].end(), c.begin(), c.end());
    }
  return h[0];
}
...
inline std::vector<T> subset_exp(const std::vector<T> &f) {
  ...
  for (int l = 1, l2 = 2; l < sz; l <<= 1, l2 = l << 1) {
    auto c = subset_convolution(ret, {f.begin() + l, f.begin() + l2});
...
T tutte_polynomial(const std::vector<std::vector<int>> &adj, T x, T y) {
  ...
  int sum[N], loops = 0, mask = ~0, s, ed, t, used[n] = {0}, lim = 2, i, j;
  ...
  std::vector<T> g = {0}, h;
  g.reserve(N), h.reserve(N);
  for (i = 0; i < n; i++) {
    h.resize(1 << i);
    for (sum[0] = j = 0; j < i; j++)
      for (s = t = 1 << j, ed = s << 1; s < ed; s++)
        sum[s] = sum[s ^ t] + adj[i][j];
    for (s = 1 << i; s--;) h[s] = g[s] * fun(sum[s]);
    auto c = subset_exp(h);
    g.insert(g.end(), c.begin(), c.end());
  }
  for (i = n; i--;)
    if (!used[i]) mask ^= 1 << i, used[i] = true, dfs(dfs, i);
  for (x -= 1, s = N; --s &= mask;) g[s] *= x;
  for (i = n; i--;) loops += adj[i][i];
  for (ret = subset_exp(g)[N - 1]; loops; loops >>= 1, y *= y)
    if (loops & 1) ret *= y;
  return ret;
}
