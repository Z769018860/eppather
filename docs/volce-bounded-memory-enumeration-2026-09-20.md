...
The 20 loop-summary comparison cases passed with identical counts and weighted MEMS on the implementation run https://github.com/Z769018860/eppather/actions/runs/35478376281. The weighted-average regression passed as well. Project-level cJSON hit the 180-second analysis limit on successive CI runs, including after restricting the new enumerator to a single memory region. The diagnostic run raised only this project's limit to 240 seconds and recorded a cJSON TIMEOUT at 240 seconds: https://github.com/Z769018860/eppather/actions/runs/35479718266. The project workflow does not enable memory projection, so the new enumerator is not selected there. For comparison, the previous implementation finished the same project workflow in about 167 seconds. The discrepancy needs stage-level profiling and an isolated baseline/branch comparison. PR #95 must not merge while its project workflow fails; these timeouts do not establish a count mismatch.
