...
The 20 loop-summary comparison cases passed with identical counts and weighted MEMS on the implementation run https://github.com/Z769018860/eppather/actions/runs/35478376281. The weighted-average regression passed as well. Project-level cJSON hit the 180-second analysis limit on successive parallel CI runs, including after restricting the new enumerator to a single memory region. The project workflow does not enable memory projection, so the new branch is not selected there. For comparison, the previous implementation took about 167 seconds in its successful cJSON project workflow. This unstable near-limit project gate must pass under an isolated retry before merging; do not label these failed runs as a proved count mismatch.
