...
## Diagnostic rerun and result policy

The original-only diagnostic rerun (Actions 36118211406) captured analyzer logs. For `list_new`, the extractor had wrongly included the text after a header's `#` in the next function, leaving an unterminated `#ifndef`; this parser-boundary bug is fixed and the regenerated C slice passes GCC syntax checking. On rerun (Actions 36118706781), `list_new` now parses as a function but exits with signal 11 during summary analysis, as do `list_rpush` and `list_lpush`. The original `ini_strncpy0` is recognized, but emits zero summary cases and solver/grammar errors for `size_t`. Original-source summary coverage is **0/4**; no valid original MaxMEMS value is available for these four functions.

The CI workflow now explicitly checks that each expected function has a numeric `validated_worst_mems` from an original native slice. A missing or model-only result fails that experiment check even when the script ran and uploaded its logs. The 5/9/9/7 instrumentation observations remain test-path witnesses, not replacements for absent static maxima. The standalone 193 collected cases remain outside this small-project diagnostic run.
